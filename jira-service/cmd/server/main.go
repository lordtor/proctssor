...
import (
	...
	"jira-service/internal/config"
	"jira-service/internal/nats"
	"jira-service/internal/registry"
	"jira-service/internal/server"
	"jira-service/internal/service"
)

func main() {
	log.Println("Starting jira-service...")
	...
	// Create Jira client
	jiraClient, err := service.NewJiraClient(
		cfg.Jira.URL,
		cfg.Jira.Username,
		cfg.Jira.Token,
		cfg.Jira.VerifyTLS,
	...
	if err != nil {
		log.Printf("Warning: Failed to create Jira client: %v", err)
		// Continue without Jira client - service can still run
	}

	// Create service handler
	handler := service.NewHandler(jiraClient)
	...
	go func() {
		httpServer.Start(startTime, consumer.IsConnected(), jiraClient != nil && jiraClient.IsConnected())
	}()
	...
	log.Println("jira-service stopped")
}
